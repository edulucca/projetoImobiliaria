...
"""
    ...
    identifiers.  The WSGI application can use those controller to get the
...
class EndpointPrefix(RuleFactory):
    """Prefixes all controller (which must be strings for this factory) with
...
class Rule(RuleFactory):
    ...
    `methods`
        ...
        controller for `POST` and `GET`.  If methods are defined and the path
...
class Map(object):
    ...
    def is_endpoint_expecting(self, endpoint, *arguments):
        ...
        code is automatically added if not provided but controller expect
        it.
